...
def svm_loss_vectorized(W, X, y, reg):
  ...
  """
    ...
    # Select one element from each row of scores using the indices in y
    ...
    # count number of incorrect classes which contributed in loss
    incorrect_class_counts = np.sum(X_mask, axis=1)
    X_mask[np.arange(num_train), y] = - incorrect_class_counts
    dW = X.T.dot(X_mask)
